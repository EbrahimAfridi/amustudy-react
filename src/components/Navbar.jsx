  ...
  return (
    <div className="sm:h-[10vh] w-[calc(100vw-8px)] flex justify-between items-center text-[15px] sm:text-[1.2rem] bg-primary border-b-[1px] border-[#1c1f26] fixed  px-4 sm:px-10 font-medium z-10">
      ...
      <div className="flex items-center gap-10">
        {post && (
          <button
          className="hidden sm:inline rounded-lg border border-transparent px-4 py-2 text-base font-medium bg-white transition-colors duration-200  focus:outline focus:outline-[4px] focus:outline-auto focus:outline-webkit-focus-ring-color"
            onClick={() => {
              if (loggedinUser !== "") {
                navigate("/new");
              }else{
